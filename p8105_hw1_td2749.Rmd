...
The given dataset consists of information regarding penguins from the years 2007-2009. It holds **`r nrow(penguins)` rows and `r ncol(penguins)` columns**. The columns contain data pertaining to _'`r colnames(penguins)`'_. Recordings for bill length, bill depth, and flipper length are made in millimeters (mm). body_mass is recorded in grams (g). The species include _'`r unique(penguins$species)`'_ from the _'`r unique(penguins$island)`'_ islands. The mean flipper length recorded for the listed penguins is **`r round(mean(nonNA_flipperlength_df$flipper_length_mm), digits = 2)` mm** (millimeters). 
